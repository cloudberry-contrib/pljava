...
@extschema@."see doc: do CREATE EXTENSION PLJAVA in new session"
(path, exnihilo) AS
SELECT CAST('libpljava-so-1.6.10' AS text), true;
LOAD 'libpljava-so-1.6.10';
GRANT USAGE ON LANGUAGE java TO public;
GRANT USAGE ON SCHEMA sqlj TO public;
GRANT SELECT ON ALL TABLES IN SCHEMA sqlj TO public;

DO $pljava$
BEGIN
  PERFORM pg_catalog.pg_stat_file(
    '/home/gpadmin/workspace/pljava/target/examples.jar'
  );
